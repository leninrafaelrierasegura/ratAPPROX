...
gets_cov_mat_rat_approx_alpha_0_to_1 <- function(graph, kappa, tau, alpha, m){
  ...
  Qtilde_i_star_UU[[i]] <- MetricGraph:::Qalpha1(
    theta = c(tau/sqrt(r[i]), sqrt(kappa^2 - p[i])), 
    graph = graph, 
    BC = 3000, 
    build = TRUE) * 1
  ...
  # A_i <- graph$.__enclos_env__$private$A()
  # 
  # # Build matrix A and Q_UU
  # A <- cbind(A_0, do.call(cbind, rep(list(A_i), m)))
  # Q_UU <- bdiag(Qtilde_0_star_UU, bdiag(Qtilde_i_star_UU))
  ...
  Q_list <- c(list(Qtilde_0_star_UU), Qtilde_i_star_UU)
  # Compute Sigma as sum of inverses using solve(Q_i, I_n)
  Sigma <- Reduce(`+`, lapply(Q_list, function(Qi) solve(Qi, I)))
  return(Sigma)
}
