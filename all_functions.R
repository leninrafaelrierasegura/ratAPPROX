...
#     # r00_inverse <- solve(r00_reg, diag(ca)) * min(diag(r00))
#     #
# 
...
# 
#   Qtilde_i <- computesListOfMatricesQTildeUnconstraint(p = p,
#                                                        kappa = kappa,
#                                                        alpha = alpha,
...
#   edgeLengths <- graph$edge_lengths
#   if(alpha == 3){
#     Qtilde <- computesListOfMatricesQTildeUnconstraint(
#     p = 0,
#     kappa = kappa,
#     alpha = alpha,
#     edgeLengths = edgeLengths)[[1]]
# 
#     K <- buildKirchooffConditioningMatrixCaseAlphaEqualThree(graph)
#     CoB <- MetricGraph:::c_basis2(K)
#     CoB$T <- t(CoB$T)
#     Tc <- CoB$T[-c(1:length(CoB$S)), ]
#     Q <- 1/sigma^2 * Tc %*% Qtilde %*% t(Tc)
#     indices <- indicesForOrderedVertices(graph, alpha = alpha)
#     A <- t(Tc)[indices, ]
#     if(build_cov) return(A %*% solve(Q, t(A)))
#     return(list(A = A, Q = Q))
#   }
# 
#   # get rational approximation coefficients
...
#   # --------------------------------------------------
# 
#   Qtilde_0 <- computesListOfMatricesQTildeUnconstraint(
#     p = 0,
#     kappa = kappa,
#     alpha = fa,
#     edgeLengths = edgeLengths)[[1]]
...
#   Tc <- CoB$T[-c(1:length(CoB$S)), ]
# 
#   inv_factor_0 <- 1/(k*c_alpha*sigma^2/c_1)
# 
#   indices <- indicesForOrderedVertices(graph, alpha = fa)
#   A_0 <- t(Tc)[indices, ]
#   Qtilde_0_star_UU <- Tc %*% Qtilde_0 %*% t(Tc) * inv_factor_0
# 
#   # --------------------------------------------------
#   # CASE i = 1,...,m
#   # --------------------------------------------------
# 
#   Qtilde_i <- computesListOfMatricesQTildeUnconstraint(
#     p = p,
#     kappa = kappa,
#     alpha = alpha,
#     edgeLengths = edgeLengths)
...
# 
#   if(alpha == 2){
#     Qtilde <- MetricGraph:::Qalpha2(
#       theta = c(tau, kappa),
#       graph = graph,
#       BC = 3000,
#       build = TRUE)
#     graph$buildC(alpha = alpha,
#                  edge_constraint = TRUE) # should always be TRUE
#     CoB <- graph$CoB
#     Tc <- CoB$T[-seq_along(CoB$S), ]
#     Q <-  Tc %*% Qtilde %*% t(Tc)
#     indices <- indicesForOrderedVertices(graph, alpha)
#     A <- t(Tc)[indices, ]
#     if(build_cov) return(A %*% solve(Q, t(A)))
#     return(list(A = A, Q = Q))
#   }
...
#   n <- 2*alpha*graph$nE
# 
#   if(is.null(graph$C)) graph$buildC(alpha = alpha, edge_constraint = TRUE)
#   K2 <- graph$C
...
#   n_times <- length(time_seq)
# 
#   row_max <- apply(cbind(U_true, U_approx), 1, max)
# 
...
#       mode = "lines",
#       x = x, y = y, z = z,
#       line = list(color = blues[i], width = 7),
#       showlegend = FALSE,
#       scene = "scene"
#     )
#   }
...
#       mode = "lines",
#       x = x, y = y, z = z,
#       line = list(color = reds[i], width = 7),#, dash = "dot"),
#       showlegend = FALSE,
#       scene = "scene"
#     )
#   }
...
# p <- p %>%
#   add_trace(
#     x = x, y = y, z = rep(z_graph, length(x)),
#     type = "scatter3d",
#     mode = "lines",
...
# 
# 
# graph.plotter.3d <- function(graph, time_seq, frame_val_to_display, U_list, log_z = FALSE) {
...
# 
#   # Set axis ranges
#   z_range <- range(unlist(U_list)) #; z_range <- c(-0.0001, z_range[2])
#   x_range <- range(x)
#   y_range <- range(y)
# 
#   z_graph <- if (log_z) min(z_range[z_range > 0]) * 0.5 else 0#min(z_range)
...
#         z = as.vector(t(cbind(z_graph, z_vals, NA))),
...
#               yaxis = list(title = "y", range = y_range),
#               zaxis = list(title = "z (log10 scale)",
#                            #range = z_range,
#                            tickformat = ".0e",
#                            type = if (log_z) "log" else "linear"   # <-- THIS is the fix
#       ),
