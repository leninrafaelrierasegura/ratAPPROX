...
```{r}
load(here::here("data_files/error_with_h0.01.RData"))
...
df_all <- dat |>
  ...
  mutate(
    ...
    method = factor(
      ...
      labels = c("Rational", "FEM")
    )
  ) |> select(-status, -starts_with("time")) |> filter(rho == "$\\rho = 0.1$") |> filter(m %in% c(1:5))

type_rational_approxVector <- c("brasil", "chebfunLB", "chebfun")
TRA <- type_rational_approxVector[1]

df <- filter(df_all, type_rational_approx == TRA)
...
errorRatFEMcomparison <- ggplot(df, aes(alpha, error, colour = m, 
  ...
  scale_linetype_manual(values = c("Rational" = "solid", "FEM" = "dashed")) + 
  scale_linewidth_manual(values = c("Rational" = 1, "FEM" = 1)) +
  scale_alpha_manual(values = c("Rational" = 0.5, "FEM" = 1.0), guide = "none") +
  theme_bw() +
  ...
  labs(x = "$\\alpha\\;(\\mbox{Smoothness parameter})$", 
       y = "$\\mbox{Covariance Error}\\quad(\\log_{10}\\mbox{ scale})$", 
       color = "$m$", 
       linetype = "Method",
       linewidth = "Method") + 
  theme(
...
myggsave(errorRatFEMcomparison, width = 8, height = 4)

# slackr_upload(
#   filename = here::here("data_files/tikzpic/errorRatFEMcomparison.pdf"),        # path to your image
#   initial_comment = paste0("Result with ", TRA),
#   channels = "#research"
# )
```
...
myggsave(errorFEMalone, width = 8, height = 4)

# slackr_upload(
#   filename = here::here("data_files/tikzpic/errorFEMalone.pdf"),        # path to your image
...
```{r, eval = TRUE, out.width="800px", out.height="400px", fig.cap = captioner("Covariance error for the tadpole graph.")}
knitr::include_graphics(here::here("data_files/tikzpic/errorRatFEMcomparison_with_mathpazo.pdf"))
```


```{r, eval = TRUE, out.width="800px", out.height="400px", fig.cap = captioner("Covariance error for the tadpole graph.")}
knitr::include_graphics(here::here("data_files/tikzpic/errorFEMalone_with_mathpazo.pdf"))
```
