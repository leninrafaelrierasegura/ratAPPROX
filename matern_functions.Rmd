...
$$
\varrho_{m, 0}^\alpha(h)=k \sigma^2 \cdot \begin{cases}\dfrac{c_\alpha \sqrt{4 \pi}}{\kappa} 1_{[h=0]}, & 0<\alpha<1 \\ \varrho\left(h ;\kappa,\lfloor\alpha\rfloor-\frac{1}{2},  \frac{c_\alpha}{c_{\lfloor\alpha\rfloor}}\right), & \alpha \geq 1\end{cases}
\tag{4}
...
$$
\varrho_{m, i}^\alpha(h)=r_i \sigma^2 \cdot \begin{cases}\varrho\left(h ;\kappa_i, \frac{1}{2},  \frac{c_\alpha \sqrt{\pi}}{\sqrt{1-p_i}}\right), & 0<\alpha<1 \\ \frac{1}{p_i^{\lfloor\alpha\rfloor}} \varrho\left(h ; \kappa_i, \frac{1}{2},  \frac{c_\alpha \sqrt{\pi}}{\sqrt{1-p_i}}\right)-\sum_{j=1}^{\lfloor\alpha\rfloor} \frac{1}{p_i^{\lfloor\alpha\rfloor+1-j}} \varrho\left(h ; \kappa, j-\frac{1}{2},  \frac{c_\alpha}{c_j}\right), & \alpha \geq 1\end{cases}
\tag{5}
...
- If `p==0`, then $\kappa_i=\kappa$ and $\frac{c_\alpha \sqrt{\pi}}{\sqrt{1-p_i}} = c_\alpha \sqrt{\pi}$. So `matern.p()` returns $\varrho(h = s-t; \kappa = \kappa_i = \kappa, \nu = \alpha-1/2, \sigma = 1)$
